...
class UserManager extends ChangeNotifier {
  ...
  Future<void> signIn({UserHR user, Function onFail, Function onSuccess}) async {
    loading = true;
    try {
...
bool  isCandidate(){
  ...
  Future<void> _loadCurrentUser({FirebaseUser firebaseUser}) async {
    ...
    if (currentUser != null) {
      ...
      final docCompany = await firestore.collection('company').document(userHR.id).get();
     if(docCompany.exists){
       userHR.accountType = 'juridica';
     }

      final docCandidate = await firestore.collection('candidate').document(userHR.id).get();
